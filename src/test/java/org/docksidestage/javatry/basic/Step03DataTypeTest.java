...
public class Step03DataTypeTest extends PlainTestCase {
     ...
     * どちらもdouble型を表します。
     */
    // done iwata [いいね] Dが大文字な理由で言うと、Dもdも両方入れてエクササイズとして紛らわしくするためです笑 by jflute (2025/08/14)
    // #1on1: そもそもレアな細かい文法を知らないと読めないコードを書かないように努力するという教訓 (2025/08/15)
    // #1on1: 縮小変換に注意、というか、やらないようにする。
    // #1on1: byte, shortはあまり使われず、int, long が主役。
    // float, doubleは誤差が生まれるので、BigDecimalに代替される。
    // char はライブラリの内部でよく使われるけど業務だとStringで十分。
    // boolean は逆にほとんどbooleanで使われる。Booleanはあまり使われない。
    // よく見るのは、int, long, boolean。
