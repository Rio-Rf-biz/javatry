...
public class Step07ExceptionTest extends PlainTestCase {
   ...
   +--------------------+        +-------------------+
    ...
    // #1on1: initCause()で使うために区別ができるようにしているようだ (2026/01/23)
    // 一方で、for/whileで番兵 (Sentinel) という言葉使うことはあるようだ。
    // https://java2005.cis.k.hosei.ac.jp/materials/lecture05/while.html
    // ただ、他で検索しても出てこない。いやこれかな？
    ...
    // #1on1: 一つの落ちたというイベントに付き、言い分 (例外 / 例外メッセージ) は複数ある。 (2026/01/23)
    /*
                 catch        catch        catch
     <---- 例外  /      例外  /     例外    /    例外
             \ /         \ /          \ /       |
  ...
  /\           |     |        PK           ^^v
               ...
               +設定ファイル (PK)
     */
    // このセオリーがわかれば、すべての例外メッセージを読もうと思うはず。
    //
    // // エラーメッセージ読め読め大合唱
    ...
    // 一方で、常に翻訳するのか？
    // 必要な箇所だけ？必要な箇所ってどこ？
    // 確かに、毎回毎回翻訳はちょっと大袈裟過ぎて手間が掛かりすぎるかも。
    // ただ、必要な場面だけってピンポイントでできるか？は難しい。
    // なので、どうしても要所要所のポイントを経験上知っていくことが大事ではある。
    // (翻訳されてなくて本番デバッグつらがる経験とか)
    // でも、その機会は少ないので難しいは難しい。でも意識してなければ一生上達しない。
    // なので、javatryのここでその意識の第一歩を踏んでもらいたい。
    //
    // 一方で一方で、そう現実は難しいので...フレームワークがある程度助けを入れてどうにかしてるって面もある。
    // LastaFluteとDBFluteの例、入り口と末端でできるだけの情報を例外メッセージに載せるようにして賄ってる。
