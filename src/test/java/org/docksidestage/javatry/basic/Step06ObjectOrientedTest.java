...
public class Step06ObjectOrientedTest extends PlainTestCase {
    ...
    public void test_objectOriented_writing_specialization_extractToConcrete() {
        ...
        // #1on1: 思考トレーニング1
        // /Users/ + loginId が Mac と Windows で同じだけど、再利用する？
        // logicが複雑なら再利用？ by いわたさん
        // 再利用するならどんな名前を付ける？
        // getHomePath()?? by いわたさん
        // ↑だとちょっと曖昧すぎる。
        // getMacAndWindowsPath()??? by いわたさん
        // ↑増えたとき減ったときどうする問題
        // /Users/の概念の捉え方
        //
        // そもそも再利用メソッドとは？何が嬉しい？
        // 片方だけ変わることがありえる再利用メソッドだと、メリットが半減
        // たまたま一緒なだけだったら、名前も付けるの困る MacAndWindows?
        // Windowsが変わって、Macだけになったら getMacPath()? それって固有メソッド。
        //
        // もし仮に ISO8857 (仮) みたいな標準仕様だったら？
        // メソッド名も明確にgetISO8857UserDirPath()
        // Windowsが独自に戻って、Macだけが呼び出してる状態になってもおかしくない。
        // こっちなら再利用メソッドのメリットあり。
        //
        // 「コードが同じだから再利用するのではなく、意味が同じだから再利用する」
        // とよく言います。
        // 意味が同じかどうか？をどう判断する？
        // ひとつのコツ: メソッド名に困るかどうか？ (概念の名前付けに困るかどうか？)
        
        // #1on1: 思考トレーニング2
        // Windows と OldWindows で getFileSeparator() が同じ、これは？
        // 同じだとして実装方法は？メソッド？定数？どこに定義する？
        // どこに定義する？が迷うようであれば、あるはずの概念がクラスになってない可能性あり。
        // abstractなWindowsに対して、OldWindowsと(New)Windows みたいな階層!?
    }
    ...
    public void test_objectOriented_zoo() {
        ...
        // _/_/_/_/_/_/_/_/_/_/
        // #1on1: ゲームの世界のゾンビだったら？
        // 仮想の世界、ゾンビの王国、ゾンビの王子様とゾンビの王女様だったら？
        //
        // 業務の独自概念の世界になったら、もっと難しい。
        // 概念化 (しっかり概念を捉える) ってのは、そんな簡単なことじゃない。
        // しっかりつど考えていかないといけない。
    }
}
